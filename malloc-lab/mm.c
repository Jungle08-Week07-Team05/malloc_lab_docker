...
#define PREV_FREE(bp) (*(void **)((char *)(bp) + WSIZE))// 그럼이것도 NEXT_FREE와 비슷하지만 한가지 다른게 bp의 시작주소 payload가 가르키는 메모리공간이 아니라 payload에다 4를 더한 주소에 prev_free를 저장하거나 읽는거야
...
static void *free_listp = NULL; //free 첫 포인터주소 선언.
...
void *mm_malloc(size_t size)
{
    ...
    for (bp=free_listp; bp!=NULL;bp=NEXT_FREE(bp)){
        if(GET_SIZE(HDRP(bp))>=asize){//for문의 bp는 freelist의 bp이고 이것들의 사이즈가 asize보다 크거나 같으면 거기에 값을 넣어줘야지
            //remove_freelist(bp); // 값을 넣어줄거니까 freelist에서 해제를 시켜줘야지
            place(bp,asize);//place도 수정을 해줬어 왜냐하면 자투리블록을 freelist에다 다시 넣어줘야 하니깐.
            return bp;
        }
    }
    // 적당한 블록이 없으면 힙 확장
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    size_t oldsize = GET_SIZE(HDRP(oldptr)) - DSIZE; // old payload 크기
...
// #define ALIGN(size) (((size) + (ALIGNMENT - 1)) & ~0x7) //0000 01111
